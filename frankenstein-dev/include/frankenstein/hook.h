...
uint32_t get_int();
asm("get_int: mrs r0, PRIMASK; CPSID I; bx lr");
void set_int(uint32_t);
asm("set_int: msr PRIMASK, r0; bx lr");
#endif
...
struct hook *__add_hook(void *target, void (*pre_hook)(struct saved_regs *regs, void *arg), uint32_t (*post_hook)(uint32_t retval, void *arg), void *arg)
{
    ...
    for (int i = 0; i < hook_code_size; i++)
        hooks[installed_hooks].code[i] = hook_code[i];
...
void post_hook(struct hook *hook, struct saved_regs *regs)
{
    ...
    // call payload
    if (hook->post_hook)
        regs->r0 = hook->post_hook(regs->r0, hook->arg);
