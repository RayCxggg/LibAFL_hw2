...
void clear_phy_status(struct saved_regs *regs, void *arg)
{
    int intmask = regs->r0;
    if (intmask & 0x7800)
        phy_status &= ~((intmask >> 8) & 0x78);
    if (intmask & 0x400000)
        phy_status &= ~HW_PHY_STATUS_RX_HEADER_DONE;
    if (intmask & 0x400)
        phy_status &= ~HW_PHY_STATUS_RX_DONE;
    if (intmask & 1)
        phy_status &= ~HW_PHY_STATUS_TX_DONE;
    if (intmask & 0xe)
        sr_status &= ~((intmask >> 1) & 0x7);
}
